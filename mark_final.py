...
img_path = os.path.join(output_dir, "original_position_map.png")
...
print(f"找到 {len(position_numbers)} 个序号")
...
for p in position_numbers:
    ...
    # 绘制序号 (红色)
    draw.text((x - 10, y - 45), f"{num}", fill="red", font=font)

    # 绘制位号 (蓝色)
    draw.text((x + 40, y - 15), tag, fill="blue", font=font)

print("\n序号 -> 位号:")
for p in position_numbers:
    num = p["num"]
    tag = tag_mapping.get(num, "N/A")
    print(f"  {num} -> {tag}")
